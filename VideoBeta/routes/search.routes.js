...
router.get('/now_playing', (req, res, next) => {
  ...
  .then (response => res.render('index', {response: response.data.results, message: "Ahora en cartelera:"}))
    .catch(error => {
      console.log(error)
    })
})
...
router.get('/populars', (req, res, next) => {
  ...
  .then (response => res.render('index', {response: response.data.results, message: "Las películas más populares:"}))
    .catch(error => {
      console.log(error)
    })
})
...
router.get('/theatres', (req, res, next) => res.render('theatres'))
